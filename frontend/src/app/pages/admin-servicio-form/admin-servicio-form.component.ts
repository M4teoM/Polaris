...
export class AdminServicioFormComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    if (id) {
      ...
      this.idEdicion = +id;
      const servicio = this.servicioService.getServicioById(this.idEdicion);
      ...
      if (servicio) {
        ...
        this.errorGeneral = 'No se encontró el servicio solicitado.';
      }
    }
  }
  ...
  guardar(): void {
    ...
    if (this.modoEdicion && this.idEdicion !== null) {
      const updated = this.servicioService.update(this.idEdicion, this.form);
      if (!updated) {
        this.errorGeneral = 'No se pudo actualizar el servicio.';
        return;
      }
    } else {
      this.servicioService.create(this.form);
    }
