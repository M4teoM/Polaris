...
export class TipoHabitacionFormComponent implements OnInit {
  ...
  ngOnInit(): void {
    void this.cargarTipoEnEdicion();
  }

  private async cargarTipoEnEdicion(): Promise<void> {
    const id = this.route.snapshot.paramMap.get('id');
    if (id) {
      this.modoEdicion = true;
      this.idEdicion = +id;
      try {
        const tipo = await firstValueFrom(
          this.tipoHabitacionService.getById(this.idEdicion),
        );
        this.form = {
          nombre: tipo.nombre,
        ...
        this.errorGeneral = 'No se pudo cargar el tipo de habitación.';
      }
    }
  }
  ...
  async guardar(): Promise<void> {
    ...
    if (this.form.precioPorNoche <= 0) {
      this.errorGeneral = 'El precio debe ser mayor a 0.';
      return;
    }
    if (this.form.metrosCuadrados <= 0) {
      this.errorGeneral = 'Los m² deben ser mayores a 0.';
      return;
    }
    ...
    if (this.modoEdicion && this.idEdicion !== null) {
      try {
        ...
        this.errorGeneral = 'No se pudo actualizar el tipo de habitación.';
      }
    } else {
      try {
        await firstValueFrom(this.tipoHabitacionService.create(this.form));
        this.router.navigate(['/admin/tipos-habitacion']);
      } catch {
        this.errorGeneral = 'No se pudo crear el tipo de habitación.';
      }
    }
  }
