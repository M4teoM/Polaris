...
export class TipoHabitacionFormComponent implements OnInit {
  ...
  form: Omit<TipoHabitacion, 'id'> = {
    ...
    precioPorNoche: 0, metrosCuadrados: 0, tipoCama: '', capacidad: 1,
  };
  ...
  ngOnInit(): void {
    ...
    if (id) {
      ...
      this.idEdicion = +id;
      this.tipoHabitacionService.getById(this.idEdicion).subscribe(tipo => {
        this.form = {
          nombre: tipo.nombre, descripcion: tipo.descripcion,
          imagenUrl: tipo.imagenUrl, precioPorNoche: tipo.precioPorNoche,
          metrosCuadrados: tipo.metrosCuadrados, tipoCama: tipo.tipoCama,
  ...
  guardar(): void {
    ...
    if (!this.form.tipoCama) { this.errorGeneral = 'Selecciona un tipo de cama.'; return; }
    if (this.form.precioPorNoche <= 0) { this.errorGeneral = 'El precio debe ser mayor a 0.'; return; }
    if (this.form.metrosCuadrados <= 0) { this.errorGeneral = 'Los m² deben ser mayores a 0.'; return; }
    if (this.form.capacidad <= 0) { this.errorGeneral = 'La capacidad debe ser mayor a 0.'; return; }

    if (this.modoEdicion && this.idEdicion !== null) {
      this.tipoHabitacionService.update(this.idEdicion, this.form).subscribe(() => {
        this.router.navigate(['/admin/tipos-habitacion']);
      });
    } else {
      this.tipoHabitacionService.create(this.form).subscribe(() => {
        this.router.navigate(['/admin/tipos-habitacion']);
      });
    }
  }
